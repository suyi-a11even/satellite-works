...
void read_header(FILE *fp) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strstr(line, "IONOSPHERIC CORR")) {
            ...
            ion.params[0] = read_fixed_double(line, 5, 12);
            ion.params[1] = read_fixed_double(line, 17, 12);
            ion.params[2] = read_fixed_double(line, 29, 12);
            ion.params[3] = read_fixed_double(line, 41, 12);
            if (strstr(ion.type, "BDS")) {
                if (strlen(line) > 53) {
                    int idx = 0;
                    int end_pos = strlen(line);
                    // 找到IONOSPHERIC CORR标记的位置，之前的就是卫星ID
                    char *corr_pos = strstr(line, "IONOSPHERIC CORR");
                    if (corr_pos) {
                        end_pos = corr_pos - line;
                    }
                    // 从53列开始读取，直到IONOSPHERIC CORR之前
                    for (int i = 53; i < end_pos && idx < 9; i++) {
                        if (line[i] != '\n' && line[i] != '\r') {
                            ion.sat_id[idx++] = line[i];
                        }
                    }
                    // 去除尾部空格
                    while (idx > 0 && ion.sat_id[idx-1] == ' ') idx--;
                    ion.sat_id[idx] = '\0';
                }
            }
...
void print_ionospheric_corr(FILE *out) {
    fprintf(out, "\n电离层参数:\n");
    fprintf(out, "%-6s %18s %18s %18s %18s  %-6s\n", "类型", "参数1", "参数2", "参数3", "参数4", "卫星ID");
    fprintf(out, "--------------------------------------------------------------------------------------------\n");
...
void print_nav_gce(FILE *out) {
    ...
    fprintf(out, "sqrt_A            e(偏心率)         i0                OMEGA0            omega             M0                ");
    fprintf(out, "delta_n           OMEGA_DOT         IDOT              Cuc               Cus               Crc               Crs               Cic               Cis               ");
    fprintf(out, "TGD1              TGD2              toe(参考时间)\n");
    for (int i = 0; i < nav_gce_count; i++) {
        ...
        // 6个基本开普勒参数
        fprintf(out, "%19.12E %19.12E %19.12E %19.12E %19.12E %19.12E ",
                n->sqrt_a, n->ecc, n->i0, n->omega0, n->omega, n->m0);
        // 9个摄动参数
        fprintf(out, "%19.12E %19.12E %19.12E %19.12E %19.12E %19.12E %19.12E %19.12E %19.12E ",
                n->delta_n, n->omega_dot, n->idot, n->cuc, n->cus, n->crc, n->crs, n->cic, n->cis);
        // 2个TGD改正数和参考时间
...
void print_nav_r(FILE *out) {
    ...
    for (int i = 0; i < nav_r_count; i++) {
        ...
        // 2个钟差参数
        fprintf(out, "%19.12E %19.12E ", n->tau_n, n->gamma_n);
        // XYZ位置
        fprintf(out, "%19.12E %19.12E %19.12E ", n->pos_x, n->pos_y, n->pos_z);
        // XYZ速度
        fprintf(out, "%19.12E %19.12E %19.12E ", n->vel_x, n->vel_y, n->vel_z);
        // XYZ加速度和频率号
